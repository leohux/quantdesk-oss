# -*- coding: utf-8 -*-
"""Gate11-style attribution for strategy-046bfa (tech breakout continuation).
...
Usage:
  python /app/scripts/surge_046bfa_attribution.py
"""
...
if Path("/app/data/research").exists():
    OUT = Path("/app/data/research/surge_046bfa_attribution.json")
    STATUS = Path("/app/data/research/strategy_status.json")
else:
    OUT = ROOT / "data" / "research" / "surge_046bfa_attribution.json"
...
def attr_block(strat: pd.Series, factors: pd.DataFrame, spy: pd.Series) -> dict:
    ...
    bench = {
        "046bfa_sleeve": metrics_from_rets(strat),
    ...
    spreads = {
        "sleeve_minus_spy_ann": bench["046bfa_sleeve"]["ann"] - bench["spy_buyhold"]["ann"],
        "sleeve_minus_spy_timing_ann": bench["046bfa_sleeve"]["ann"]
        - bench["spy_ma200_timing"]["ann"],
        "sleeve_minus_mtum_ann": bench["046bfa_sleeve"]["ann"] - bench["mtum_buyhold"]["ann"],
        "sleeve_minus_spy_sharpe": bench["046bfa_sleeve"]["sharpe"] - bench["spy_buyhold"]["sharpe"],
        "sleeve_minus_spy_timing_sharpe": bench["046bfa_sleeve"]["sharpe"]
        - bench["spy_ma200_timing"]["sharpe"],
        # classify() looks for ma_trend_minus_spy_timing_ann
        "ma_trend_minus_spy_timing_ann": bench["046bfa_sleeve"]["ann"]
        - bench["spy_ma200_timing"]["ann"],
    }
    ...
    gate14 = {
        ...
        "checks": {
            ...
            "sleeve_sharpe_gt_spy": bool(
                bench["046bfa_sleeve"]["sharpe"] > bench["spy_buyhold"]["sharpe"]
            ),
...
def main() -> int:
    ...
    out = {
        ...
        "prior_evidence": {
            "is_oos_artifact": "data/research/surge_046bfa_is_oos.json",
            "is_oos_gates_pass": False,
            "signal_decay_0bp": {
                "sharpe_train_valid_holdout": [0.191, 0.152, 0.098],
                "win_rate_train_valid_holdout": [0.533, 0.521, 0.464],
            },
        },
    }
    ...
    if STATUS.exists():
        ...
        status["strategies"][SID] = {
            ...
            else "data/research/surge_046bfa_attribution.json",
        }
        status["updated_at"] = datetime.now(timezone.utc).strftime("%Y-%m-%d")
        next_list = status.setdefault("system", {}).setdefault("next", [])
        note = "046bfa: attribution done — research_archive_gate_fail; do not FB/trail-optimize"
...
def _print_block(block: dict) -> None:
    ...
    print(
        f"sleeve ann={b['046bfa_sleeve']['ann']:.1%} "
        f"S={b['046bfa_sleeve']['sharpe']:.2f} DD={b['046bfa_sleeve']['maxdd']:.1%}",
        flush=True,
    )
    print(
