# -*- coding: utf-8 -*-
"""IS / Valid / Holdout + cost sensitivity for Surge-NVDA (EOD surge cousin).

Uses store strategy_code + params (buy_surge/cap, trend_ma=20, max_hold_days=4).
Equal-weight average of per-symbol run_signal_backtest metrics.

Splits (aligned with 046bfa / rr_grid_surge):
...
Usage:
  python /app/scripts/surge_nvda_is_oos.py
"""
...
COST_BPS = (0, 5, 10, 20)
OUT = Path("/app/data/research/surge_nvda_is_oos.json")
if not Path("/app/data/research").exists():
    OUT = Path(__file__).resolve().parents[1] / "data" / "research" / "surge_nvda_is_oos.json"
